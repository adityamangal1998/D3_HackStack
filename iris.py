...
with mp_face_mesh.FaceMesh(
 ...
 # 2 Approach = color Contour
        ...
        print(len(contours))
        otsu = np.zeros((700, 700), np.uint8)
        for i in contours:
            # epsilon = 0.1 * cv.arcLength(i, True)
            # approx = cv.approxPolyDP(i, epsilon, True)
            rect = cv.minAreaRect(i)
            box = cv.boxPoints(rect)
            box = np.int0(box)
            otsu = cv.drawContours(otsu, [box], -1, (255, 0, 0), 1)
            print(box)
            cropped = cv.resize(frame,box[2],interpolation=cv.INTER_NEAREST)
