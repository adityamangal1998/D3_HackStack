            ...
            for face_landmarks in results.multi_face_landmarks:
                for idx, lm in enumerate(face_landmarks.landmark):
                    if idx == 1:
                        ...
                        center = (x, y)
                        img = cv2.circle(img, center, radius, (255, 0, 0), thickness)
                img = cv2.flip(img, 1)
                if (125 < y < 175 and x < 125) or (y > 175 and x < 125 and y >= x) or (125 > y >= x and x < 125):
                    head_pos = "Looking Right"
                    cv2.putText(img, "Looking Right", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                elif (125 < y < 175 and x > 175) or (175 < y <= x and x > 175) or (y < 125 and x > 175 and y <= x):
                    head_pos = "Looking Left"
                    cv2.putText(img, "Looking Left", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                elif (125 < x < 175 and y > 175) or (175 < x <= y and y > 175) or (x < 125 and y > 175 and y >= x):
                    head_pos = "Looking Down"
                    cv2.putText(img, "Looking Down", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                elif (125 < x < 175 and y < 125) or (x > 175 and y < 125 and y <= x) or (125 > x >= y and y < 125):
                    head_pos = "Looking Up"
                    cv2.putText(img, "Looking Up", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                elif 125 < y < 175 and 125 < x < 175:
                    head_pos = "Forward"
                    cv2.putText(img, "Forward", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
